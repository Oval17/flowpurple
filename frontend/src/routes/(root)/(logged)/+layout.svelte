<script lang="ts">
	...
	// FlowPurple personal build: no AI entry points in the rail — hide both the
	// Workspace ⇄ Sessions switch and the legacy Ask-AI button.
	const FLOWPURPLE_LITE = true
	const sessionsSwitchShown = $derived(
		!FLOWPURPLE_LITE && globalAiEnabled && !$copilotInfo.workspaceDisabled
	)
...
<!-- FlowPurple brand mark anchoring the sidebar bottom (the header slot is taken
...
{#snippet brandMark(collapsed: boolean)}
	<div class="flex items-center gap-x-1.5 text-xs font-semibold text-emphasis">
		...
		{#if !collapsed}
			{$whitelabelNameStore ? capitalize($whitelabelNameStore) : 'FlowPurple'}
		{/if}
	</div>
{/snippet}
...
{:else if $userStore}
	...
	<div class="h-screen flex flex-col">
		...
		<div class="flex flex-col h-full w-full">
			...
			<!-- FlowPurple personal build: docked AI chat hidden (disableAi). All pages
			     render through the {:else} branch of AiChatLayout unchanged. -->
			<AiChatLayout
				{children}
