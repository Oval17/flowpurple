<script lang="ts">
	...
	// FlowPurple personal build: minimal sidebar. Main nav keeps Home + Runs only,
	// the Triggers section (Schedules + all trigger links) is hidden, audit/workers/
	// logs entries and the external Help links are dropped. Pages stay reachable by URL.
	const FLOWPURPLE_LITE = true
	const FLOWPURPLE_LITE_MAIN = ['Home', 'Runs']
	const FLOWPURPLE_LITE_HIDDEN_SECONDARY = ['Workers', 'Logs', 'Audit logs']
	...
	let mainMenuLinks = $derived(
		[
			...
			(l) =>
				!excludeMainLabels.includes(l.label) &&
				(!FLOWPURPLE_LITE || FLOWPURPLE_LITE_MAIN.includes(l.label))
		)
	)
	...
	let secondaryMenuLinks = $derived([
		...
		(l) => !FLOWPURPLE_LITE || !FLOWPURPLE_LITE_HIDDEN_SECONDARY.includes(l.label)
	))
</script>
