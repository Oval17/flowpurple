...
- New name/identity: **FlowPurple** (`@flowpurple/components`, tab titles, login, sidebar brand, loader)
- Full purple-and-white theme: light surfaces are white / `#F5F3FF`, accents are `#7C3AED → #6D28D9 → #5B21B6`, dark mode is a deep purple (`#1E1B2E`), rounder component corners, purple gradient login card
- Simplified UI (`FLOWPURPLE_LITE` flags): sidebar keeps Home + Runs; Schedules, triggers, Variables/Resources/Assets/Folders/Groups, Workers, Logs, Audit logs, external Help links, docked AI chat, AI Sessions switch, "Build with AI" hero, and Hub pickers are hidden (pages still work by direct URL)
- Purple logo + loading screen (`frontend/static/logo.svg`, `frontend/src/app.html`, `WindmillIcon.svelte`)
...
docker compose up -d
# open http://localhost:8080  (login: admin@windmill.dev — default password `changeme`, rotate it after first boot)
```
...
cd frontend
npm install
REMOTE=http://localhost:8080 npm run dev
# open the printed http://localhost:3000 URL (or whichever port vite picks)
```
